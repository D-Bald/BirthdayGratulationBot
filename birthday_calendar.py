...
async def get_todays_birthdays(guild_id: int):
    ...
    Returns:
        All birthdays as pandas DataFrame with columns `name`, `date`, `guild_id` and `dates_left`.
    """
    df = await get_birthdays(guild_id)
...
async def add_entry(name, date: str, guild_id: int):
    ...
    Returns:
        The added birthday as pandas DataFrame with columns `name`, `date`, `guild_id` and `days_left`.
    """
    df_new = pd.DataFrame({"name": [name], "date": [utils.format_date_string(date)], "guild_id": guild_id, "days_left": days_left(date)})
    df_dates = pd.concat(
        [await _get_all_birthdays(), df_new],
        ignore_index=True,
    )
...
async def remove_entry(name: str, guild_id: int):
    ...
    Args:
        name: the name of the person as string
    """
    df_dates = await get_birthdays(guild_id)
